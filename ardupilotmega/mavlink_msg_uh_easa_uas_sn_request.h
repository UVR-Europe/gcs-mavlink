...
typedef struct __mavlink_uh_easa_uas_sn_request_t {
 ...
 uint8_t cmd; /*<  0 - get, 1 - set UAS serial number*/
 char serial_number[20]; /*<  4 chars mfrr code, 1 char hex length code, 1-15 chars mfrr number*/
} mavlink_uh_easa_uas_sn_request_t;

#define MAVLINK_MSG_ID_UH_EASA_UAS_SN_REQUEST_LEN 25
#define MAVLINK_MSG_ID_UH_EASA_UAS_SN_REQUEST_MIN_LEN 25
#define MAVLINK_MSG_ID_16003_LEN 25
#define MAVLINK_MSG_ID_16003_MIN_LEN 25

#define MAVLINK_MSG_ID_UH_EASA_UAS_SN_REQUEST_CRC 197
#define MAVLINK_MSG_ID_16003_CRC 197
...
#define MAVLINK_MESSAGE_INFO_UH_EASA_UAS_SN_REQUEST { \
    ...
    {  { "request_id", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_uh_easa_uas_sn_request_t, request_id) }, \
         ...
         { "serial_number", NULL, MAVLINK_TYPE_CHAR, 20, 5, offsetof(mavlink_uh_easa_uas_sn_request_t, serial_number) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_UH_EASA_UAS_SN_REQUEST { \
 ...
 * @param cmd  0 - get, 1 - set UAS serial number
...
{
    return mavlink_msg_uh_easa_uas_sn_request_pack(system_id, component_id, msg, uh_easa_uas_sn_request->request_id, uh_easa_uas_sn_request->cmd, uh_easa_uas_sn_request->serial_number);
}
...
{
    return mavlink_msg_uh_easa_uas_sn_request_pack_chan(system_id, component_id, chan, msg, uh_easa_uas_sn_request->request_id, uh_easa_uas_sn_request->cmd, uh_easa_uas_sn_request->serial_number);
}
...
static inline void mavlink_msg_uh_easa_uas_sn_request_send(mavlink_channel_t chan, uint32_t request_id, uint8_t cmd, const char *serial_number)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_UH_EASA_UAS_SN_REQUEST_LEN];
    _mav_put_uint32_t(buf, 0, request_id);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_uh_easa_uas_sn_request_send(chan, uh_easa_uas_sn_request->request_id, uh_easa_uas_sn_request->cmd, uh_easa_uas_sn_request->serial_number);
#else
 ...
 */
static inline void mavlink_msg_uh_easa_uas_sn_request_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t request_id, uint8_t cmd, const char *serial_number)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 *
 * @return  0 - get, 1 - set UAS serial number
 */
...
{
    return _MAV_RETURN_char_array(msg, serial_number, 20,  5);
}
